...
import { ArrowRight, BookOpen, Search, User, Plus, X, Paperclip, CheckCircle, Trash2 } from 'lucide-react';
    ...
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
          <div className="my-8 w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-lg font-bold text-slate-950">Write a Research Article</h2>
                <p className="mt-1 text-xs text-slate-500">Submitted articles are published once an admin approves them.</p>
              </div>
              <button onClick={() => setShowModal(false)} className="rounded-full p-1 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button>
            </div>
